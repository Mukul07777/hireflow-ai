...
**Component tests:** `src/components/ui.test.jsx` renders the shared UI primitives with
`react-dom/server` and asserts real failure modes — `ProgressBar` clamping out-of-range values
instead of emitting invalid CSS, `Btn`/`Tag` falling back safely on unknown variants, `Toast`
rendering every type without throwing. No browser or extra dependency required.

> **A real bug this suite caught:** the test run surfaced Postgres error `42P17 —
> "infinite recursion detected in policy for relation employees"` on *every* database call.
> The multi-tenancy policies checked membership by selecting from `employees` inside a policy
> *on* `employees`, so evaluating the policy re-triggered it forever — silently breaking every
> insert and select in production. Fixed in [`supabase/07_rls_recursion_fix.sql`](supabase/07_rls_recursion_fix.sql)
> by moving the membership lookup into `SECURITY DEFINER` functions, the standard Supabase
> pattern for self-referential membership tables.
...
- **`App.jsx` is still a single ~5,950-line file.** The logic libraries (`companyBrain`, `brainContext`, `modeAccess`, `groqKeyRotation`, `resumeParsing`, `indiaComplianceRules`, `indianVerification`, `outcomeLearning`, `costEstimator`) and the shared UI primitives (`components/ui.jsx`) are extracted and tested, but the individual agent screens are not yet split out. Splitting them is in progress.
...
Or individually, in order:
  ...
  07_rls_recursion_fix.sql   <- fixes the 42P17 policy recursion; do not skip
```
